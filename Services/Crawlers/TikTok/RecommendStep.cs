...
namespace UzonMail.ProPlugin.Services.Crawlers.TikTok
{
    ...
    {
        private static readonly ILog _logger = LogManager.GetLogger(typeof(RecommendStep));
        ...
        protected async Task CrawlFolloers(TiktokAuthor authorInfo)
        {
            ...
            while (followerInfo != null && !ShouldStop())
            {
                ...
                var existFollower = await _db
                    .TiktokAuthors.Where(x => x.Id == followerId)
                    .FirstOrDefaultAsync();
                ...
                if (existFollower != null)
                {
                    ...
                    // 1.递归复制既有结果
                    await TikTokEmailCrawler.CopyCrawledResultsRecursively(
                        _db,
                        authorInfo,
                ...
                {
                    // 保存粉丝信息
                    var followersCrawler = new FollowersStep(
                        crawlerTaskParams,
                        authorInfo.Id,
                        followerInfo,
                        followerId
