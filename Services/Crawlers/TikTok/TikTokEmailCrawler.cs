...
namespace UzonMail.ProPlugin.Services.Crawlers.TikTok
{
    ...
    public class TikTokEmailCrawler : CrawlerTaskBase
    {
        ...
        public TikTokEmailCrawler(IServiceProvider serviceProvider)
        ...
        /// <returns></returns>
        private async Task StartRecommendStep(
            JObject recommendVideoInfo,
        ...
        )
        {
            ...
            // 判断是否存在
            var existAuthor = await _db
                .TiktokAuthors.Where(x => x.Id == authorInfo.Id)
                .FirstOrDefaultAsync();
            ...
            {
                // 1.递归复制既有结果
                await CopyCrawledResultsRecursively(
                    _db,
                    authorInfo,
                    crawlerTaskParams.CrawlerTaskId
                );
            ...
            else
            {
                // 任务未进行，添加新任务
                // 开启新任务
                var recommendStep = new RecommendStep(
                    crawlerTaskParams,
                    recommendVideoInfo,
                    authorInfo.Id
                );
        ...
        public static async Task CopyCrawledResultsRecursively(
            SqlContextPro db,
            TiktokAuthor authorInfo,
            long crawlerTaskId
        )
        {
            if (authorInfo == null)
            ...
            // 判断是否存在
            var scrawlerResult = await db
                .CrawlerTaskResults.Where(x =>
                    x.CrawlerTaskInfoId == crawlerTaskId && x.TikTokAuthorId == authorInfo.Id
                )
                .FirstOrDefaultAsync();
