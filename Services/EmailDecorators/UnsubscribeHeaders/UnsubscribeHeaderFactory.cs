namespace UzonMail.ProPlugin.Services.EmailBodyDecorators.UnsubscribeHeaders
{
    ...
    {
        private static Dictionary<string, IUnsubscribeHeader> _unsubscribeHeadersDic =
            new() { { "rfc8058", new RFC8058Header() } };
        ...
        /// <returns></returns>
        public static IUnsubscribeHeader GetUnsubscribeHeader(
        ...
        )
        {
            ...
            // 判断是否指定了退订头部
            var header = unsubscribeConfig.Headers.Find(x =>
                x.Domain.Equals(type, StringComparison.CurrentCultureIgnoreCase)
            );
            if (header != null)
                type = header.Domain.ToLower();
