...
namespace UZonMail.ProPlugin.Controllers.Unsubscribes
{
    ...
    ) : ControllerBasePro
    {
        ...
        /// <returns></returns>
        [HttpPost()]
        public async Task<ResponseResult<UnsubscribePage>> CreateUnsubscribePage(
            [FromBody] UnsubscribePage unsubscribePage
        )
        {
            if (string.IsNullOrEmpty(unsubscribePage.Language))
                return ResponseResult<UnsubscribePage>.Fail("languageRequired");
            if (string.IsNullOrEmpty(unsubscribePage.HtmlContent))
                return ResponseResult<UnsubscribePage>.Fail("htmlContentRequired");
        ...
        public async Task<ResponseResult<bool>> UpdateUnsubscribePage(
            long id,
            [FromBody] UnsubscribePage unsubscribePage
        )
        {
            ...
            await dbPro.UnsubscribePages.UpdateAsync(
                x => x.OrganizationId == organizationId && x.Id == id,
                x => x.SetProperty(y => y.HtmlContent, htmlContent)
            );
        ...
        {
            var organizationId = tokenService.GetOrganizationId();
            var exist = await dbPro.UnsubscribePages.FirstOrDefaultAsync(x =>
                x.OrganizationId == organizationId && x.Id == id
            );
            if (exist == null)
                return true.ToSuccessResponse();
        ...
        [AllowAnonymous]
        [HttpGet()]
        public async Task<ResponseResult<UnsubscribePage>> GetUnsubscribePage(
            string token,
            long id,
            string language
        )
        {
            ...
            if (id > 0)
            {
                pageResult = await dbPro
                    .UnsubscribePages.Where(x => x.Id == id)
            ...
            if (pageResult == null && !string.IsNullOrEmpty(language))
            {
                // 从语言中获取
                pageResult = await dbPro
                    .UnsubscribePages.Where(x => x.Language == language)
                    .Where(x => x.OrganizationId == organizationId || x.IsDefault)
                    .FirstOrDefaultAsync();
            }
            // 如果未找到，则从默认的退订页面中获取
            pageResult ??= await dbPro
                .UnsubscribePages.Where(x => x.IsDefault || x.OrganizationId == organizationId)
                .FirstOrDefaultAsync();
        ...
        public async Task<ResponseResult<int>> GetUnsubscribePagesCount(string filter)
        {
            var organizationId = tokenService.GetOrganizationId();
            var dbSet = dbPro
                .UnsubscribePages.AsNoTracking()
                .Where(x => x.OrganizationId == organizationId || x.IsDeleted);
            if (!string.IsNullOrEmpty(filter))
            {
                dbSet = dbSet.Where(x =>
                    x.Language.Contains(filter) || x.HtmlContent.Contains(filter)
                );
            }
        ...
        public async Task<ResponseResult<List<UnsubscribePage>>> GetUnsubscribePagesData(
            string filter,
