...
{
    public class UnsubscribeItemFilter(SqlContext sqlContext, SqlContextPro dbPro) : ISendingItemFilter
    {
        public async Task<List<long>> GetInvalidSendingItemIds(List<SendingItem> sendingItems)
        {
            ...
            // 对于取消订阅的邮件，进行标记
            var userInfo = await CacheManager.Global.GetCache<UserInfoCache>(sqlContext, sendingItems[0].UserId);
            var unsubscribedEmails = await GetUnsubscribedEmails(userInfo.OrganizationId);
            if (unsubscribedEmails.Count == 0) return [];

            var unsubscribedSendingItemIds = sendingItems.Where(x => x.Inboxes.Any(i => unsubscribedEmails.Contains(i.Email))).Select(x => x.Id).ToList();
        ...
        {
            var unsubscribedEmails = await dbPro.UnsubscribeEmails.AsNoTracking()
